...
(function () {
  ...
  async function countLikes(fanUserId) {
    if (!likesApi?.countUserGivenLikes) {
      return null;
    }

    try {
      return await likesApi.countUserGivenLikes(fanUserId);
    } catch (error) {
      console.error("countLikes failed:", error);
      return null;
    }
  }
  ...
  function setLikesUi(value, message) {
    const statLikes = document.getElementById("statLikes");
    const statLikesHint = document.getElementById("statLikesHint");

    if (statLikes) {
      statLikes.textContent = value == null ? "—" : String(value);
    }

    if (statLikesHint) {
      statLikesHint.textContent = message || "";
    }
  }
  ...
  async function boot() {
    ...
    try {
      ...
      if (likesCount == null) {
        setLikesUi("—", "Unavailable");
      } else {
        setLikesUi(likesCount, "");
      }
